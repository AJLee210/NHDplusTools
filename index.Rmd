...
![Example of flowlines and catchments.](pics/watershed.png)
...
![Schematic of HUC units, taken from the USGS, https://www.usgs.gov](pics/HUCS.png)
...
# you can also pull a HUC-8 using a point or polygon feature 
...
```{r}
flowlines_for_pathlengths <- get_tocomid(nofo_flowlines, add=TRUE) %>% rename(ID=comid, toID=tocomid) # this function oddly uses a different naming convention than what's provided 
                                 # in the NHD

# creates a table containing the distance between each flowline and our network's outlet.
distances <- get_pathlength(flowlines_for_pathlengths) %>% 
    rename(comid=ID, distance_km=pathlength)
...
for(i in 1:nrow(gages)){
  ...
  } # for every gage (row, "i") in our dataset, create a comid column and fill it with that gage's comid
```
...
```{r}
mapview(passage_creek_flowlines) + mapview(passage_creek_watershed) + mapview(passage_creek, col.regions='red')
```
...
```{r}
trace <- get_raindrop_trace(passage_creek) # traces the NHDPlus digital elevation model to the nearest downslope NHD flowline
...
```{r}
# extract comid from the outlet 
main_stream <- get_UT(network = nofo_flowlines, comid = 8441253)

# categorized gages using comid of main stream
gages_main <- filter(gages, comid %in% c(main_stream)) 

gages_tribut <- filter(gages,!comid %in% c(main_stream))

# mapview
mapview(gages_main, col.regions='red') + mapview(gages_tribut) + mapview(nofo_flowlines) + mapview(nofo_watershed, col.regions='white')
...
```{r}
# Hint from the class, get the distance
main_stream <- get_UT(network = nofo_flowlines, comid = 8441253) %>% 
               as_tibble() %>% 
               rename(comid=value)

main_stream_network <- filter(nofo_flowlines, comid %in%                                     main_stream$comid) %>% 
                       get_tocomid(., add=TRUE) %>% 
                       rename(ID=comid, toID=tocomid)

distance_main_stream <- get_pathlength(main_stream_network) %>% 
                        rename(comid=ID)

my_distance <- get_pathlength(main_stream_network) %>% 
               filter(ID==8441037) %>% 
               rename(comid=ID)

# mapping
## for gages
NF_gages <- filter(gages, station_nm == c('N F SHENANDOAH RIVER NEAR STRASBURG, VA','N F SHENANDOAH RIVER AT COOTES STORE, VA'))

## for distance between the gages
main_stream2 <- get_UT(network = nofo_flowlines, comid = 8441037) %>% 
                as_tibble() %>% 
                rename(comid=value)

final_main_stream_comid <- main_stream %>% 
                           filter(., !comid %in% main_stream2$comid)

final_main_stream_comid <- rbind(final_main_stream_comid, 8441037)

map_distance <- inner_join(nofo_flowlines, final_main_stream_comid,                              by='comid') %>% 
                filter(., streamorde==4)

mapview(main_stream_network)+mapview(NF_gages)+mapview(map_distance, color='red')
...
```{r}
# using 'get_UT()' function
## make comid of the flows between the two gages
NF_Star  <- filter(gages, station_nm == 'N F SHENANDOAH RIVER NEAR STRASBURG, VA')

NF_Coot <- filter(gages, station_nm == 'N F SHENANDOAH RIVER AT COOTES STORE, VA')

NF_UT_comids <- get_UT(network = nofo_flowlines, comid=NF_Star$comid) %>% as_tibble() %>% rename(comid=value)

NF_UT_comids2 <- get_UT(network = nofo_flowlines, comid=NF_Coot$comid) %>% as_tibble() %>% rename(comid=value)

NF_UT_comids_final <- NF_UT_comids[!NF_UT_comids$comid %in% c(NF_UT_comids2$comid),]

## get the distance between two gages using above comid final
NF_river_flowlines <- filter(nofo_flowlines, comid %in% 
                      c(NF_UT_comids_final$comid), streamorde==4)

NF_river_flowlines_path <- get_tocomid(NF_river_flowlines, add=T)                           %>% rename(ID=comid, toID=tocomid) 

Distance_getUT <- get_pathlength(NF_river_flowlines_path) %>% 
                  rename(comid=ID, distance_km=pathlength)

Distance_getUT

mapview(NF_river_flowlines_path) + mapview(NF_gages)

# using 'get_DM()'
## make comid between two gages
NF_DM_comids <- get_DM(network = nofo_flowlines, comid=NF_Coot$comid) %>% as_tibble() %>% rename(comid=value)

NF_DM_comids2 <- get_DM(network = nofo_flowlines, comid=NF_Star$comid) %>% as_tibble() %>% rename(comid=value)

NF_DM_comids_final <- NF_DM_comids[!NF_DM_comids$comid %in% c(NF_DM_comids2$comid),]

# main flow between tow gages using above comid final, and distance
NF_flowlinesD <- filter(nofo_flowlines, comid %in% 
                      c(NF_DM_comids_final$comid), streamorde==4)

NF_flowlinesD_path <- get_tocomid(NF_river_flowlines, add=T)                           %>% rename(ID=comid, toID=tocomid) 

Distance_getDM <- get_pathlength(NF_flowlinesD_path) %>% 
                  rename(comid=ID, distance_km=pathlength)

Distance_getDM

mapview(NF_flowlinesD_path) + mapview(NF_gages)
...
**A: As shown the table below, the distance of #2a question is 130.306km, while the one of 2b and 2c is 130.266 km. So, the two results are slightly different, as 0.04km. I think the reason of the difference came from if the flow path where the upper gage(comid=8441037) is included or not, as you can find the different each comid number in the table. **

```{r}
# check the distance result of each question
my_distance
Distance_getDM[29,]

# make a table to compare clearly
distance_table <- data.frame(my_distance, Distance_getDM[29,]) 

colnames(distance_table) <-                                    c('2a_comid','distance(km)','2bc_comid','distance(km)')

distance_table %>% 
   kable(.,'html',caption='Distance between the gages') %>%
   kable_styling() %>%
   scroll_box(width='900px',height='400px')
...
```{r}
#AOI
Vrain_watershed <- get_huc8(id='10190005')

Vrain_flowlines <- get_nhdplus(AOI = Vrain_watershed,                                    realization='flowline')
#gage
Vrain_gages <- get_nwis(AOI=Vrain_watershed)

mapview(Vrain_gages) + mapview(Vrain_flowlines)+mapview(Vrain_watershed)

# select gages within watershed boundaries
Vrain_gages <- Vrain_gages %>% 
               filter(st_intersects(., Vrain_watershed, sparse = F))

mapview(Vrain_gages) + mapview(Vrain_flowlines)+mapview(Vrain_watershed)

# Distance of flowlines from the outlet
Vrain_flowlines_path <- get_tocomid(Vrain_flowlines, add=T) %>% 
                        rename(ID=comid, toID=tocomid)

Vdistance <- get_pathlength(Vrain_flowlines_path) %>% 
             rename(comid=ID, distance_km=pathlength)

Vrain_flowlines <- left_join(Vrain_flowlines, Vdistance, by='comid')

# Make comid of gage and Join with the features of Flowlines
for(i in 1:nrow(Vrain_gages)){
  Vrain_gages$comid[i] <- discover_nhdplus_id(Vrain_gages[i,])
  }

Vrain_gages <- left_join(Vrain_gages, Vdistance, by='comid')

Vflowlines_for_gage <- data.frame(select(Vrain_flowlines, c('comid','streamorde','areasqkm'))) 

Vflowlines_for_gage <- Vflowlines_for_gage[,1:3]

Vrain_gages <- inner_join(Vrain_gages, Vflowlines_for_gage, by='comid')
...
```{r}
#  get cross section using 'get_xs_point' function
## select interested point, gage in N ST VRAIN CREEK AT WILD BASIN TRAIL NR ALLENSPARK
point <- sf::st_sfc(sf::st_point(x = c(-105.5669444, 40.2072222)), crs = 4326)

## using the function (point, width, points along the cross section)
